...
- **Your collection**: items, purchase prices, quantities, dates, notes, organisation. Stored in a local database inside the app's private storage.
- **Your item photos**: saved in the app's private storage. They are never uploaded anywhere and are never analysed — photo framing happens entirely on the device.
- **Your PIN** (if you enable App Lock): kept in the system's secure storage (Android Keystore). It never leaves the device.
...
- the app offers full data deletion from its settings;
- uninstalling the app erases all of its local data;
